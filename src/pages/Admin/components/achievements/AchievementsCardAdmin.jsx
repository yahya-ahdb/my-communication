...
function AchievementsCardAdmin( { title, img, id ,content } ) {

    const headers = useHeaders()
    const navgite = useNavigate()
    const goToDetalis = () => {
        navgite( `/Achievements_Details`, { state: { title, img, id } } )
    }
    const goToEdite = () => {
        navgite( `/admin-EditAchievements`, { state: { title, img, id,content } } )
    }
    ...
    const handelDelete = async () => {
        setLoaing( true )
        await axios.delete( apiUrl + `deleteAchievement/${id}`, { headers: headers } ).then( () => {
    ...
    return (
        <div className='achievements-card' style={ { maxWidth: "300px", display: "flex", flexDirection: "column", background: "#F7F8FB" } }>
            ...
            <Dialog open={ open } onClose={ handleClose } >
                <DialogTitle style={ { direction: "rtl" } }>حذف الإنجاز { title }</DialogTitle>
                <DialogContent>
                    <DialogContentText style={ { marginBottom: "30px", direction: "rtl" } }>
                        إن عملية الحذف هذه لا يمكن الرجوع عنها وستفقد معلومات الإنجاز
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
...
export default AchievementsCardAdmin