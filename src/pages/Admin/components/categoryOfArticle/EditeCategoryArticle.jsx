...
export default function EditeCategoryArticle( { title, id } ) {
    ...
    const handleEditeCategroy = async () => {
        setLoaing( true )
        await axios.post( apiUrl + `updateArticleCategory/${id}`, { "value": titleState },{headers: { ...headers, "Content-Type": "multipart/form-data" }} ).then( () => {
